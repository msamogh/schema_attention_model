...
def evaluate(
    model, eval_dataloader, schema_dataloader, tokenizer, task, device=0, args=None
):
    ...
    if args.use_schema and task == "action":
        with torch.no_grad():
            ...
            sc_all_output, sc_pooled_output = model.bert_model(
                input_ids=sc_batch["input_ids"],
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        with torch.no_grad():
            if task == "action":
                ...
                sentence += [
                    tokenizer.decode(e.tolist(), skip_special_tokens=False).replace(
                        " [PAD]", ""
                    )
...
def train(args, exp_setting=None):
    ...
    tokenizer = BertWordPieceTokenizer(args.token_vocab_path, lowercase=True)
    tokenizer.enable_padding(length=args.max_seq_length)

    sc_tokenizer = BertWordPieceTokenizer(args.token_vocab_path, lowercase=True)
    ...
    dataset = dataset_initializer(
        args.data_path, tokenizer, args.max_seq_length, token_vocab_name
    )
    # dataset.examples = [e for e in dataset if 'weather' in e['tasks']]

    # Get the action to id mapping
    if args.task == "generation":
        action_dataset = NextActionDataset(
            args.data_path, tokenizer, args.max_seq_length, token_vocab_name
        )
    ...
    if exp_setting is not None:
        if "domain" in exp_setting:
            ...
            data_type = exp_setting.get("data_type")
            train_dataset = filter_dataset(
                dataset, data_type=data_type, percentage=0.8, train=True
            )

            test_dataset = filter_dataset(
                dataset, data_type=data_type, percentage=0.2, train=False
            )
    ...
    schema_train_dataloader = DataLoader(
        dataset=schema,
    ...
    schema_test_dataloader = DataLoader(
        dataset=schema, batch_size=len(schema), pin_memory=True, shuffle=True
    )

    test_dataloader = DataLoader(
        dataset=test_dataset, batch_size=args.train_batch_size, pin_memory=True
    )
        ...
        for epoch in trange(args.num_epochs, desc="Epoch"):
            ...
            for batch in tqdm(train_dataloader):
                ...
                if args.use_schema:
                    ...
                    batch_action_tasks = [
                        (batch["action"][i].item(), batch["tasks"][i])
                    ...
                    sc_batch = {
                        k: [v[i] for i in relevant_inds]
                        if type(v) is list
                        else v[relevant_inds]
